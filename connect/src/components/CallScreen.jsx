...
export default function CallScreen({ name, roomName, onEnded }) {
  ...
  return <section className="call-layout"><div className="call-main"><div className="call-top"><button className="back-button" onClick={onEnded} aria-label="Return to conversations">‹</button><div><p className="eyebrow">SECURE VOIP CALL</p><h1>{name}</h1></div><ConnectionStatus status={status} /></div><div className="call-identity"><div className="large-avatar">{name.slice(0, 1).toUpperCase()}</div><p><i className="presence online" /> {status === 'connected' ? 'Connected' : 'Connecting'}</p><div className="call-clock">{duration}<small>CALL DURATION</small></div></div>{error ? <div className="call-error" role="alert"><strong>Call unavailable</strong><p>{error}</p><button onClick={retryConnection}>RETRY CONNECTION</button><button onClick={onEnded}>RETURN TO JOIN</button></div> : <div className="call-waiting">{status === 'connected' ? 'Microphone connected' : 'Establishing a secure connection'}</div>}<CallControls muted={muted} onMute={toggleMute} onEnd={endCall} /></div><aside><ParticipantList participants={participants} /></aside></section>
}